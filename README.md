...
- **Re-adding a contact from the original distribution key** restarts at offset 0 over already-spent bytes. `--add-contact` now warns when a contact name has been used on this keychain before, but a warning is not a guarantee - only supply key material that has never been used.
...
| None of the above | The artifact's recorded range cannot be reconciled with the key file and the metadata - or the key file cannot be read at all | Discard it without redelivering, and say so. Guessing which range was really spent is the one thing recovery must never do; the run then proceeds normally from the state that *is* known. |
